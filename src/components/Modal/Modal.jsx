...
const Modal = ({ onCloseModal, car }) => {
  ...
  return createPortal(
    <Overlay onClick={handleClickBackdrop}>
      <ModalWindow>
        <ButtonClose onClick={handleClickButton}><OutlineClose size={24} /></ButtonClose>
        <Image src={car.img} />
        <Title>
        ...
        <List>
          {conditionsArray.map((condition, index) => (
            <Item key={index}>{condition.replace(/\d+/g, '')} 
            {condition.match(/\d+/g)?.map((number, numIndex) => (
              <SpanNumber key={numIndex}>{number}</SpanNumber>
            ))}</Item>
          ))}
          <Item>Mileage: <SpanNumber>{car.mileage}</SpanNumber> </Item>
          <Item>Price: <SpanNumber>{car.rentalPrice}</SpanNumber></Item>
        </List>
